...
struct Notification {
    int id;
    string name;
    string appearance; // height, body build, clothing, distinguishing features
    string lastLocation;
    string crimeType;
    bool read;
    string timestamp;
...
void displayNotificationCenter(string userLocation) {
    ...
    for (int i = filtered.size() - 1; i >= 0; i--) { // Most recent first
    ...
    cout << "Enter notification number to view details (1-" << filtered.size() << "), or 0 to exit: ";
    int choice;
    cin >> choice;
    if (choice > 0 && choice <= filtered.size()) {
        Notification n = filtered[filtered.size() - choice];
...
void showPopupNotification(Notification n) {
    ...
    MessageBox(NULL, message.c_str(), "SafeWatch Alert", MB_OK | MB_ICONINFORMATION);
}
